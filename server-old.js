...
io.on('connection',function(socket){
    ...
    socket.on('start',function(){
        ...
        socket.on('death',function(data){
            players[data.id - 1] = false;
            io.emit('death',data.id); //In future, implement more complex handling, like death screen, respawn time, ghost modes
        });

        socket.on('disconnect',function(){
            players[socket.player.id - 1] = false;
